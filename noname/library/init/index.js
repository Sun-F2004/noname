...
import compiler from "../element/GameEvent/compilers/ContentCompiler.js";
class LibInit {
  #promises;
  /**
   * 部分函数的Promise版本
          ...
          navigator.notification.confirm(
            ...
            function(index) {
              if (index == 2) {
                ...
                var onlineKey2 = localStorage.getItem(lib.configprefix + "key");
                localStorage.clear();
                if (noname_inited) {
                  localStorage.setItem("noname_inited", noname_inited);
                }
                if (onlineKey2) {
                  localStorage.setItem(lib.configprefix + "key", onlineKey2);
                }
                if (indexedDB) {
...
扩展文件：${pathToRead}`;
  ...
  req(str, onload, onerror, master) {
    ...
    fetch(sScriptURL).then((result) => result.text()).then(onload).catch(onerror);
  }
  json(url, onload, onerror) {
    fetch(url).then((result) => result.json()).then(onload).catch(onerror);
  }
  cssstyles() {
    if (ui.css.styles) {
  ...
  parseResourceAddress(link, defaultHandle = null, loadAsDataUrlCallback = null, dbNow = false) {
    ...
    if (loadAsDataUrlCallback != null) {
      if (resultUrl.protocol == "db:") {
        game.getDB("image", linkString.slice(3)).then(
          (storeResult) => this.parseResourceAddress(storeResult, defaultHandle, loadAsDataUrlCallback, true)
        );
      } else {
        get.blobFromUrl(resultUrl).then((blob) => get.dataUrlAsync(blob)).then(loadAsDataUrlCallback);
      }
    }
    return resultUrl;
  }
