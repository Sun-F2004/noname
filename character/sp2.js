...
const skills = {
    ...
    async content(event, trigger, player) {
      ...
      for (const target of targets) {
        const result = await target.chooseControl({
          ...
          choiceList: skills2.map(
            (skill2) => `<div class="skill">【${get.translation(lib.translate[`${skill2}_ab`] || get.translation(skill2).slice(0, 2))}】</div><div>${get.skillInfoTranslation(skill2, player, false)}</div>`
          ),
          displayIndex: false,
          prompt: "选择获得一个技能"
        }).forResult();
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          ...
          if (target.mayHaveShan(player, "use") && player.hasCards(
            "hs",
    ...
    async content(event, trigger, player) {
      ...
      const result = await player.chooseCardTarget({
        ...
        ai1: (card) => {
          ...
          if (get.tag(card, "damage") && game.hasPlayer(
  ...
  spjiedao: {
    ...
    trigger: { source: "damageBegin1" },
    filter(event, player) {
      return player.isDamaged() && game.getGlobalHistory(
        "everything",
        (evt) => evt.name === "damage" && evt.source === player,
        event
      ).indexOf(event) === 0 && event.player.isIn();
    },
    logTarget: "player",
  ...
  xpchijie2: {
    ...
    filter(event, player) {
      return event.player !== player && event.targets.includes(player) && event.cards.filterInD().length > 0 && !game.hasPlayer2(
        (current) => current.getHistory("damage", (evt) => evt.card === event.card).length > 0
      );
    },
    usable: 1,
    check(event, player) {
